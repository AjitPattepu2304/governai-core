...
public class RiskEvaluationService {
    public Evaluation evaluate(AIApplication app) {
        ...
        int overall = Math.round((privacy + security + fairness + transparency + regulatory) / 5.0f);
        ...
        return new Evaluation(privacy, security, fairness, transparency, regulatory, overall, AssessmentRiskLevel.fromScore(overall), explanation.toString());
    }
    ...
    private int scoreFairness(AIApplication a) {
        ...
        if (a.getAiType() == AIType.PREDICTIVE_ML || a.getAiType() == AIType.RECOMMENDATION) score += 15;
        if (a.getHumanOversight() == HumanOversight.NONE) score += 20;
        else if (a.getHumanOversight() == HumanOversight.HUMAN_APPROVAL) score -= 10;
        if (a.getLifecycle() == Lifecycle.PRODUCTION) score += 10;
        return clamp(score);
    }
    ...
    private int scoreTransparency(AIApplication a) {
        ...
        if (a.getHumanOversight() == HumanOversight.NONE) score += 25;
        else if (a.getHumanOversight() == HumanOversight.HUMAN_APPROVAL) score -= 10;
    ...
    private int impactPoints(DecisionImpact impact) {
        return switch (impact) { case LOW -> 0; case MEDIUM -> 10; case HIGH -> 25; case CRITICAL -> 40; };
    }

    private int clamp(int score) { return Math.max(0, Math.min(100, score)); }

    public record Evaluation(int privacy, int security, int fairness, int transparency, int regulatory,
                             int overall, AssessmentRiskLevel riskLevel, String explanation) {}
}
