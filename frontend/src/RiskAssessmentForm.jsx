        ...
        <form className="risk-run-form" onSubmit={evaluate}>
          <div className="risk-info-card">
            ...
            <div>
              ...
              <p>GovernAI will evaluate this system using the registered context and save an auditable assessment with methodology version.</p>
            </div>
          </div>
          {error && <div className="error">{error}</div>}
          <div className="form-actions">
            <button type="button" className="secondary" onClick={onCancel}>Cancel</button>
            <button className="primary" disabled={saving}>{saving ? 'Evaluating...' : 'Run Risk Evaluation →'}</button>
          </div>
        </form>
      )}
                ...
                return (
                  <div className="risk-score-card" key={key}>
                    <div className="risk-score-top">
                      <span>{name}</span>
                      <span className={`risk-mini-status risk-mini-${tone}`}>{tone.toUpperCase()}</span>
                    </div>
                    <div className="risk-score-number"><strong>{score}</strong><span>/ 100</span></div>
                    <div className="risk-score-track"><span className={`risk-score-fill risk-fill-${tone}`} style={{ width: `${Math.min(score, 100)}%` }} /></div>
                  </div>
                );
              })}
            </div>
